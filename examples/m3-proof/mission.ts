  ...
  if (process.env.MISSURA_LIVE !== "1") {
    fail(
      "refusing to run: this is a live proof against real vendor APIs.\n" +
        ...
        "  missura exec --entity <type:name> --repo <owner/name> --purpose 'm3 proof' -- pnpm demo:m3\n" +
...
export function requireToken(): string {
  ...
  if (token.length === 0) {
    fail(
      "no mission token — run this under: missura exec --entity <type:name> --repo <owner/name> --purpose 'm3 proof' -- pnpm demo:m3",
    );
  }
  return token;
}
